...
public class HKSPSService {
	...
	public void pull() throws Exception {
		...
		for (int index = 0; index < records.length(); index++) {
			...
			if (tokens[1] != null) {
				...
				List<PSEntity> list = schoolJpaRepository.findBySchoolName(schoolName);
				if (list.size() > 0) {
					try {
						for(PSEntity pSEntity : list) {
							schoolJpaRepository.save(pull(schoolId, pSEntity));
	...
	private PSEntity pull(String schoolId, PSEntity pSEntity) throws IOException {
