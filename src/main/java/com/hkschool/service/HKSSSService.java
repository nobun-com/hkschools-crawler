...
public class HKSSSService {
	...
	public void pull() throws Exception {
		...
		for (int index = 0; index < records.length(); index++) {
			...
			if (tokens[1] != null) {
				...
				List<SSEntity> list = schoolJpaRepository.findBySchoolName(schoolName);
				if (list.size() > 0) {
					try {
						for(SSEntity sSEntity : list) {
							schoolJpaRepository.save(pull(schoolId, sSEntity));
	...
	private SSEntity pull(String schoolId , SSEntity sSEntity) throws IOException {

		Document doc = Jsoup.connect("https://www.schooland.hk/ss/" + schoolId).get();
		Elements rows = doc.getElementsByClass("row");
		
		Element row = rows.get(1);
		//String schoolDescription = row.getElementsByTag("p").get(0).text();
		...
		//sSEntity.setSchoolDiscritpion(schoolDescription);
